...
async def fetch_valid_members(chat_id):
    valid_users = []
    async for member in app.get_chat_members(chat_id):
        if not member.user.is_bot and not member.user.is_deleted:
            valid_users.append(member.user)
    return valid_users

async def execute_user_tagging(chat_id, text_list, process_name):
    target_users = await fetch_valid_members(chat_id)
    
    for u in target_users:
        if chat_id not in active_tagging_processes:
            break
            
        user_mention = f"<b><a href='tg://user?id={u.id}'>{u.first_name}</a></b>"
        dispatch_msg = random.choice(text_list).format(mention=user_mention)
        
        await app.send_message(chat_id, dispatch_msg, disable_web_page_preview=True)
        await asyncio.sleep(3)
    
    active_tagging_processes.pop(chat_id, None)
    await app.send_message(chat_id, f"✅ <b>{process_name} Tᴀɢɢɪɴɢ Dᴏɴᴇ!</b>")


@app.on_message(filters.command("gmtag") & filters.group)
async def trigger_morning_tag(_, message: Message):
    chat_id = message.chat.id
    
    if chat_id in active_tagging_processes:
        return await message.reply("⚠️ <b>Gᴏᴏᴅ Mᴏʀɴɪɴɢ Tᴀɢɢɪɴɢ Aʟʀᴇᴀᴅʏ Rᴜɴɴɪɴɢ.</b>")
    
    active_tagging_processes[chat_id] = True
    await message.reply("☀️ <b>Gᴏᴏᴅ Mᴏʀɴɪɴɢ Tᴀɢɢɪɴɢ Sᴛᴀʀᴛᴇᴅ...</b>")
    
    await execute_user_tagging(chat_id, MORNING_TEXTS, "Gᴏᴏᴅ Mᴏʀɴɪɴɢ")

@app.on_message(filters.command("gmstop") & filters.group)
async def halt_morning_tag(_, message: Message):
    chat_id = message.chat.id
    
    if chat_id in active_tagging_processes:
        del active_tagging_processes[chat_id]
...
async def trigger_afternoon_tag(_, message: Message):
    chat_id = message.chat.id
    ...
    await execute_user_tagging(chat_id, AFTERNOON_TEXTS, "Aғᴛᴇʀɴᴏᴏɴ")

@app.on_message(filters.command("gastop") & filters.group)
async def halt_afternoon_tag(_, message: Message):
    chat_id = message.chat.id
...
async def trigger_night_tag(_, message: Message):
    chat_id = message.chat.id
    ...
    await execute_user_tagging(chat_id, NIGHT_TEXTS, "Gᴏᴏᴅ Nɪɢʜᴛ")

@app.on_message(filters.command("gnstop") & filters.group)
async def halt_night_tag(_, message: Message):
    chat_id = message.chat.id
...
async def force_stop_all_tags(_, message: Message):
    chat_id = message.chat.id
...
async def display_tag_help(_, message: Message):
    guide_text = """
