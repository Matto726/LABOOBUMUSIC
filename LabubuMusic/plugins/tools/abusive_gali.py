...
ABUSIVE_STRINGS = [
    ...
    "𝗧𝗘𝗥𝗜 𝗕𝗔𝗛𝗘𝗡 𝗞𝗜 𝗖𝗛𝗨𝗨́𝗧 𝗠𝗘𝗜 𝗔𝗣𝗡𝗔 𝗕𝗔𝗗𝗔 𝗦𝗔 𝗟𝗢𝗗𝗔 𝗚𝗛𝗨𝗦𝗦𝗔 𝗗𝗨𝗡𝗚𝗔𝗔 𝗞𝗔LL𝗔𝗔𝗣 𝗞𝗘 𝗠𝗔𝗥 𝗝𝗔𝗬𝗘𝗚𝗜 🤩😳😳🔥",
...
@app.on_message(filters.command(["gali", ".gali"], prefixes=["/", "."]) & filters.private)
async def send_gali_private(client: Client, message: Message):
    await message.reply_text(text=random.choice(ABUSIVE_STRINGS))

@app.on_message(filters.command(["gali", ".gali"]) & filters.group)
async def send_gali_group(client: Client, message: Message):
    user_id = message.from_user.id
    target_chat = message.chat.id
    
    try:
        user_status = await client.get_chat_member(target_chat, user_id)
    except Exception:
        return
        
    if user_status.status in ["administrator", "creator"]:
        if message.reply_to_message:
            reply_resp = await message.reply(f"{message.reply_to_message.from_user.mention} {random.choice(ABUSIVE_STRINGS)}")
        else:
            reply_resp = await message.reply(random.choice(ABUSIVE_STRINGS))
        
        await asyncio.sleep(60)
        try:
            await reply_resp.delete()
            await message.delete()
        except Exception:
            pass
    else:
        reply_resp = await message.reply("**🚫 This command is only for admins!**\n\n💬 Try this in my DMs instead.")
